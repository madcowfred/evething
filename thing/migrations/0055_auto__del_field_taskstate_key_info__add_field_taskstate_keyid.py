...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'TaskState.key_info'
        db.delete_column('thing_taskstate', 'key_info')

        # Adding field 'TaskState.keyid'
        db.add_column('thing_taskstate', 'keyid',
                      self.gf('django.db.models.fields.IntegerField')(default=0, db_index=True),
                      keep_default=False)

        # Adding index on 'TaskState', fields ['parameter']
        db.create_index('thing_taskstate', ['parameter'])


    def backwards(self, orm):
        # Removing index on 'TaskState', fields ['parameter']
        db.delete_index('thing_taskstate', ['parameter'])

        # Adding field 'TaskState.key_info'
        db.add_column('thing_taskstate', 'key_info',
                      self.gf('django.db.models.fields.CharField')(default=0, max_length=80, db_index=True),
                      keep_default=False)

        # Deleting field 'TaskState.keyid'
        db.delete_column('thing_taskstate', 'keyid')
