...
urlpatterns += patterns('thing.views',
    ...
    (r'^skillplan/ajax/entries/(?P<skillplan_id>\d+)/(?P<character_id>\d+)/(?P<implants>\d+)/(?P<show_trained>\d+)/$','skillplan_render_entries'),
    (r'^skillplan/ajax/entries/reorder/(?P<skillplan_id>\d+)/(?P<skill_entry>\d+)/(?P<new_position>\d+)/$','skillplan_reorder_entry'),
