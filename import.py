...
def main():
	# Connect databases
	inconn = sqlite3.connect('inc110-sqlite3-v1.db')
	incur = inconn.cursor()
	...
	for row in incur:
		outcur.execute('INSERT INTO thing_itemcategory (id, name) VALUES (?, ?)', row)
	outconn.commit()
	...
	for row in incur:
		outcur.execute('INSERT INTO thing_itemgroup (id, name, category_id) VALUES (?, ?, ?)', row)
	outconn.commit()
	print '%.1fs' % (time.time() - t)
	...
	for row in incur:
		outcur.execute("INSERT INTO thing_item (id, name, group_id, portion_size, volume, sell_price, buy_price) VALUES (?, ?, ?, ?, ?, 0, 0)", row)
	outconn.commit()
	print '%.1fs' % (time.time() - t)
